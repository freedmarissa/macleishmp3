...
weather_stat <- data.frame(lat = 42.449653, lon = -72.680315)
weather_stat <- data.frame(Lat = 42.448470, lon = -72.680553)
```
...
research_buffer_final <- research_buffer2 %>%
  st_join(research_buffer, left = FALSE)
rbuffers_inside <- research_buffer_final %>%
  st_intersection(macleish_research)
...
allbuffers_inside2 <- sbuffers_inside %>%
  st_intersection(rbuffers_inside)
